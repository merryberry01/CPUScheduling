 ...
 */
TCB* kCreateTask( QWORD qwFlags, QWORD qwEntryPointAddress, int iRRTestTaskFlag, int iProcTime )
{
    TCB* pstTask;
    void* pvStackAddress;
    ...
    pstTask->iRRFlag = iRRTestTaskFlag;
...
void kInitializeScheduler( void )
{
    ...
    gs_stScheduler.pstRunningTask->iRRFlag = 0;
...
BOOL kScheduleInInterrupt( void )
{
    ...
    // 테스트 태스크인데 프로세스의 남아 있는 실행 시간이 0일 때 대기 리스트에 삽입
    if( pstRunningTask->qwFlags & TASK_FLAGS_ENDTASK ||
    (pstRunningTask->iRRFlag == 0xAB && pstRunningTask->iProcTime <= 0))
    {    
        kAddListToTail( &( gs_stScheduler.stWaitList ), pstRunningTask );
    }
...
void kDecreaseProcessorTime( void )
{
    ...
    if( gs_stScheduler.pstRunningTask->iRRFlag == 0xAB && gs_stScheduler.pstRunningTask->iProcTime > 0)
    {
        gs_stScheduler.pstRunningTask->iProcTime--;
    }
}
