...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
{
        ...
        { "createrrtask", "Create RR Task ex) createrrtask 3(count) 50(ms) 40(ms) 30(ms)", kCreateRRTestTask },
};                                     
 ...
 * RR create task
 */
static void kCreateRRTestTask( const char* pcParameterBuffer )
{
    PARAMETERLIST stList;
    char vcCount[ 30 ];
    char vcTime[ 30 ];
    ...
    for(i = 0; i < kAToI(vcCount, 10); i++){
        ...
        kCreateTask( TASK_FLAGS_HIGHEST, (QWORD) kTestTaskRR, 0xAB, kAToI(vcTime, 10));
