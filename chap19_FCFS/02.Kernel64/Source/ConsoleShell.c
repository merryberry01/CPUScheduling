...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
{
        ...
        { "createfcfstask", "Create FCFS Task ex) createfcfstask 3(count) 50(ms) 40(ms) 30(ms)", kCreateFCFSTestTask },
};                                     
...
static void kCreateTestTask( const char* pcParameterBuffer )
{
    ...
    case 1:
        ...
        {    
            if( kCreateTask( TASK_FLAGS_HIGHEST, ( QWORD ) kTestTask1, 0, TASK_PROCESSORTIME ) == NULL )
            {
                break;
            }
        }
    ...
    default:
        for( i = 0 ; i < kAToI( vcCount, 10 ) ; i++ )
        {    
            if( kCreateTask( TASK_FLAGS_HIGHEST, ( QWORD ) kTestTask2, 0, TASK_PROCESSORTIME ) == NULL )
            {
                break;
            }
        }
 ...
 *      자신의 ID를 참고하여 특정 위치에 회전하는 바람개비를 출력
 */
static void kTestTaskFCFS( void )
{
    int i = 0, iOffset;
    CHARACTER* pstScreen = ( CHARACTER* ) CONSOLE_VIDEOMEMORYADDRESS;
    TCB* pstRunningTask;
    ...
    kPrintf("Running Task's job jength: %dms\n", pstRunningTask->iProcTime);
    while( 1 )
    {
        // 회전하는 바람개비를 표시
        pstScreen[ iOffset ].bCharactor = vcData[ i % 4 ];
 ...
 * FCFS create task
 */
static void kCreateFCFSTestTask( const char* pcParameterBuffer )
{
    PARAMETERLIST stList;
    char vcCount[ 30 ];
    char vcTime[ 30 ];
    ...
    for(i = 0; i < kAToI(vcCount, 10); i++){
        kGetNextParameter(&stList, vcTime);
        kCreateTask( TASK_FLAGS_HIGHEST, (QWORD) kTestTaskFCFS, 0xAB, kAToI(vcTime, 10));
