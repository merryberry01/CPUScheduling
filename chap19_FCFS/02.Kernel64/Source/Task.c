 ...
 */
TCB* kCreateTask( QWORD qwFlags, QWORD qwEntryPointAddress, int iFCFSTestTaskFlag, int iProcTime)
{
    TCB* pstTask;
    void* pvStackAddress;
    ...
    pstTask->iFCFSFlag = iFCFSTestTaskFlag;
...
void kInitializeScheduler( void )
{
    ...
    gs_stScheduler.pstRunningTask->iFCFSFlag = 0;
...
BOOL kScheduleInInterrupt( void )
{
    ...
    // FCFS Test Task일 경우 대기 리스트에 삽입
    if( ( pstRunningTask->qwFlags & TASK_FLAGS_ENDTASK ) || pstRunningTask->iFCFSFlag == 0xAB )
    {
        kAddListToTail( &( gs_stScheduler.stWaitList ), pstRunningTask );
    }
