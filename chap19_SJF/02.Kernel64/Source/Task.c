 ...
 */
TCB* kCreateTask( QWORD qwFlags, QWORD qwEntryPointAddress, int iSJFTestTaskFlag, int iProcTime)
{
    TCB* pstTask;
    void* pvStackAddress;
    ...
    pstTask->iSJFFlag = iSJFTestTaskFlag;
    pstTask->iProcTime = iProcTime;
...
void kInitializeScheduler( void )
{
    ...
    gs_stScheduler.pstRunningTask->iProcTime = TASK_PROCESSORTIME;
    gs_stScheduler.pstRunningTask->iSJFFlag = 0;
...
TCB* kGetNextTaskToRun( void )
{
    ...
    for( j = 0 ; j < 2 ; j++ )
    {
        ...
        for( i = 0 ; i < TASK_MAXREADYLISTCOUNT ; i++ )
        {
            ...
            {
                TCB* pstNode;
                int iMinProcTime = 1 << 30;
                QWORD qwMinID;
                for(pstNode = kGetHeaderFromList(&(gs_stScheduler.vstReadyList[i])); pstNode != NULL; pstNode = (TCB*)(pstNode->stLink.pvNext)){
                    if(iMinProcTime > pstNode->iProcTime){
                        iMinProcTime = pstNode->iProcTime;
                        qwMinID = pstNode->stLink.qwID;
                    }
                }
                pstTarget = ( TCB* ) kRemoveList( &( gs_stScheduler.vstReadyList[ i ] ), qwMinID );
...
BOOL kScheduleInInterrupt( void )
{
    ...
    // SJF Test Task일 경우 대기 리스트에 삽입
    if( ( pstRunningTask->qwFlags & TASK_FLAGS_ENDTASK ) || pstRunningTask->iSJFFlag == 0xAB )
    {
        kAddListToTail( &( gs_stScheduler.stWaitList ), pstRunningTask );
    }
    ...
    // 프로세서 사용 시간을 업데이트
    gs_stScheduler.iProcessorTime = pstNextTask->iProcTime;
    return TRUE;
}
    ...
    while( 1 )
    {
        ...
        if( kGetListCount( &( gs_stScheduler.stWaitList ) ) >= 0 )
        {
            while( 1 )
            {
                ...
                kPrintf( "IDLE: (%d) Task ID[0x%q] is completely ended.\n", 
                        pstTask->iProcTime, pstTask->stLink.qwID );
